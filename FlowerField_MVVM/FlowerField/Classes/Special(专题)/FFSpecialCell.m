...
- (void)handleData {
    ...
    @weakify(self)
    [RACObserve(self, cellModel) subscribeNext:^(FFSpecialViewModel  *viewModel) {
        @strongify(self)
        [self.pictureView yy_setImageWithURL:[NSURL URLWithString:viewModel.smallIcon] placeholder:[UIImage imageNamed:@"placehodler"]];
        ...
        self.identityLabel.text = viewModel.authorIdentity;
        ...
        [self.bottomView.readBtn setTitle:viewModel.readNum forState:UIControlStateNormal];
        [self.bottomView.followBtn setTitle:viewModel.followNum forState:UIControlStateNormal];
        [self.bottomView.commentBtn setTitle:viewModel.commentNum forState:UIControlStateNormal];
        
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] init];
        [self.headImgView addGestureRecognizer:tap];
        [[tap rac_gestureSignal] subscribeNext:^(id x) {
            [viewModel.headerClickSubject sendNext:x];
        }];;
